...
from ..tools.selection.selection import SelectionSync
...
class ClayersPanel:
    ...
    def _build_visibility_cell(self, item: _Item) -> None:
        ...
        def _click() -> None:
            # Items inside a locked group are non-interactive.
            if self._path_inside_locked_group(path):
                return
    ...
    def _build_lock_cell(self, item: _Item) -> None:
        ...
        if item.prim is None:
            ...
            return
        lock_mgr = LockManager.get()
        own_locked = lock_mgr.is_locked_self(item.prim) if lock_mgr is not None else False
        group_locked = self._path_inside_locked_group(item.path)
        locked = own_locked or group_locked
        icon = _ICON_LOCKED if locked else _ICON_UNLOCKED
        image = self._centered_icon(icon, _LOCK_WIDTH, _ICON_VISUAL, centered=True)
        prim = item.prim
        path = item.path
        ...
        def _click() -> None:
            # When an ancestor group is locked, items are non-interactive
            # regardless of their own lock state.
            if self._path_inside_locked_group(path):
                return
            mgr = LockManager.get()
            if mgr is None:
    ...
    # TRUE IF THIS USD PATH BELONGS TO A GROUP (OR NESTED PARENT GROUP)
    # WHOSE OWN LOCK FLAG IS SET. ITEMS INSIDE A LOCKED GROUP MUST NOT
    # BE TOGGLABLE FROM THE CLAYERS UI, REGARDLESS OF THEIR OWN LOCK STATE.
    def _path_inside_locked_group(self, path: str | None) -> bool:
        if not path:
            return False
        groups = GroupRegistry.get()
        if groups is None:
            return False
        gid = groups.group_of_path(path)
        seen: set[str] = set()
        # Walk up through parent groups. parent_of returns either another
        # group id or an anchor string; anchors have no name so we use that
        # as the stop condition.
        while gid is not None and gid not in seen:
            seen.add(gid)
            if groups.is_locked_self(gid):
                return True
            parent = groups.parent_of(gid)
            if parent is None or groups.get_name(parent) == "":
                return False
            gid = parent
        return False
