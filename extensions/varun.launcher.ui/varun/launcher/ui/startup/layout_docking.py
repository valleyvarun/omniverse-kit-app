...
class LayoutDocking:
    ...
    def add_window_menu_items(self) -> None:
        async def _apply_default_layout() -> None:
            # Manual re-apply: always run the full first-run-style load so
            # the user's layout snaps back to the JSON exactly.
            await self._startup_defaults.load_layout(first_run=True)
